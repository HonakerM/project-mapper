...
pub fn process(comp_type: CompType, input: ImplInput, args: ImplArgs) -> TokenStream {
    let item_impl = input.implementation.clone();
    let mut expanded = quote! {
        #item_impl
    };
    let type_name = type_name(&input.implementation.self_ty).unwrap();

    let config_expr = args.config_expr.clone();
    let src_expr = if let Some(src) = args.src_expr {
        quote! {#src}
    } else {
        quote! {project_mapper_runtime::project_mapper_core::runtime_config::effect::common::DefaultSrcConfig::default()}
    };

    let config_val = match comp_type {
    ...
    let requires_refresh_expr = if let Some(expr) = args.requires_refresh_expr {
        quote! { #expr}
    } else {
        quote! { false }
    };

    let available_config_val = match comp_type {
        CompType::Input => {
            let available_config_expr = if let Some(expr) = args.available_expr {
                quote! {
                    #expr
                }
            } else {
